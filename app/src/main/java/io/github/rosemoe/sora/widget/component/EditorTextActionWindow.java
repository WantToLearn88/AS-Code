...
public class EditorTextActionWindow extends EditorPopupWindow implements View.OnClickListener, EditorBuiltinComponent {
    ...
    protected void onEditorFocusChange(  EditorFocusChangeEvent event) {
        if (!event.isGainFocus()) {
            dismiss();
        }
    }

    protected void onEditorRelease(  EditorReleaseEvent event) {
        setEnabled(false);
    }

    protected void onEditorLongPress(  LongPressEvent event) {
    ...
    protected void onEditorScroll(  ScrollEvent event) {
        var last = lastScroll;
    ...
    protected void onHandleStateChange(  HandleStateChangeEvent event) {
        if (event.isHeld()) {
            postDisplay();
        }
    ...
    protected void onSelectionChange(  SelectionChangeEvent event) {
