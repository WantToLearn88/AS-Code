...
public class EditorSearcher {
     ...
     */
    public void search(  String pattern,   SearchOptions options) {
        if (pattern.isEmpty()) {
    ...
    public void replaceThis(  String replacement) {
     ...
     * @throws IllegalStateException if no search is in progress
     */
    public void replaceAll(  String replacement,   final Runnable whenSucceeded) {
        if (!editor.isEditable()) {
            return;
        }
        checkState();
    ...
    private final class SearchRunnable implements Runnable {
        ...
        public SearchRunnable(  Content content,   SearchOptions options,   String pattern) {
