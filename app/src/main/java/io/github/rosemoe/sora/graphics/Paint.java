...
public class Paint extends android.graphics.Paint {
    ...
    @SuppressLint("NewApi")
    public float myGetTextRunAdvances(  char[] chars, int index, int count, int contextIndex, int contextCount, boolean isRtl,   float[] advances, int advancesIndex, boolean fast) {
        if (fast) {
            ensureCacheObject();
            var width = 0f;
